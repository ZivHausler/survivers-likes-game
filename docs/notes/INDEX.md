...
- [[arena-scatter]] — `ArenaScatter`: static `compute_positions()` (deterministic seeded rejection-sampling placement on XZ; respects extent/clear-radius/min-separation) + instance-side `_ready` spawner that scatters sci-fi pylon/barrier primitive props as `Obstacle3D`s under an `Obstacles` node (exported tunable params; BoxMesh fallback on load failure)
- [[arena-map]] — `arena_3d.tscn`: Final City arena — seamless pavement ground (`final_city_pavement_albedo.png`, albedo-only, `uv1_scale=(60,60,60)`, tint `Color(0.7,0.72,0.78)`), HDRI mountain sky; border walls, water ponds, and seeded collidable sci-fi pylon/barrier prop scatter
- [[sci-fi-pylon]] — `obstacles/sci_fi_pylon_3d.tscn`: tall sci-fi pylon prop (~6 units); dark metal body + cyan/magenta emissive rings; visual-only `Node3D` (no collision); fills the pylon slot in ArenaScatter (even indices)
- [[sci-fi-barrier]] — `obstacles/sci_fi_barrier_3d.tscn`: low sci-fi barrier prop (~2 units); dark metal block + cyan emissive top stripe; visual-only `Node3D`; fills the barrier slot in ArenaScatter (odd indices)
