...
- [[arena-scatter]] — `ArenaScatter.compute_positions()`: deterministic, seeded rejection-sampling placement of obstacles on XZ plane; respects extent, clear-radius, and min-separation; gracefully handles over-dense requests
- [[arena-map]] — `arena_3d.tscn`: realistic arena map — PBR grass ground (Poly Haven aerial_grass_rock CC0), HDRI mountain sky (Poly Haven kloofendal_43d_clear_puresky CC0); obstacles/walls/water layered by later tasks
